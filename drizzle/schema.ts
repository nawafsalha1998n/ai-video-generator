...
/**
 * Video generation requests and results
 */
export const videos = mysqlTable("videos", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().references(() => users.id),
  requestId: varchar("requestId", { length: 255 }).notNull().unique(),
  type: mysqlEnum("type", ["text-to-video", "image-to-video"]).notNull(),
  prompt: text("prompt").notNull(),
  imageUrl: text("imageUrl"),
  videoUrl: text("videoUrl"),
  status: mysqlEnum("status", ["pending", "done", "failed", "expired"]).default("pending").notNull(),
